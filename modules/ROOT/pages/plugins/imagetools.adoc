...
The Image Tools plugin is provided with all subscriptions to link:{baseurl}/cloud-deployment-guide/editor-and-features.html[TinyMCE Cloud], including an automatically configured image proxy.
...
NOTE: `imagetools_cors_hosts` is *not* required when enabling this plugin via link:{baseurl}/cloud-deployment-guide/editor-and-features.html[TinyMCE Cloud].
...
https://www.tinymce.com/pricing/[TinyMCE Enterprise] subscription also includes proxy service written in Java. Check link:{baseurl}/enterprise/server/index.html[Install Server-side Components] guide for details.

NOTE: `imagetools_proxy` is *not* required when enabling this plugin via link:{baseurl}/cloud-deployment-guide/editor-and-features.html[TinyMCE Cloud]
