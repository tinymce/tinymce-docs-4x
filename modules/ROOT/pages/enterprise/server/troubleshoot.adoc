...
. The `application.conf` file is correct, and the services are working, but the server-side component URLs that the editor uses are not quite right. Refer to the link:{baseurl}/plugins/tinymcespellchecker.html[Spell Checker Pro], link:{baseurl}/plugins/imagetools.html[Image Tools], link:{baseurl}/plugins/mediaembed.html[Enhanced Media Embed] and link:{baseurl}/plugins/linkchecker.html[Link Checker] plugin pages for help.
