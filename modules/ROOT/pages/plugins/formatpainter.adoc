...
Format Painter is a premium plugin from Tiny. Please see the link:{baseurl}/enterprise/formatpainter.html[Premium features] section for all the buying options.
...
The format painter acts upon all link:{baseurl}/configure/content-formatting/#formats[`formats`] registered in the editor. In addition to any standard formatting, it will treat *lists* as a block format whenever the link:{baseurl}/plugins/lists.html[`lists plugin`] is made available.
...
----
Check out the link:{baseurl}/demo/formatpainter.html[Format Painter demo] to try out this new feature.
